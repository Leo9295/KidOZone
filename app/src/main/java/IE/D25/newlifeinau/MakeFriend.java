...
public class MakeFriend extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new slideAdapter5(this);
    ...
    private void showDialog() {
        ...
        builder.setNeutralButton("False", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                Display display = getWindowManager().getDefaultDisplay();
                int height = display.getHeight();
                Toast toast = Toast.makeText(MakeFriend.this, "Great!", Toast.LENGTH_LONG);
                toast.setGravity(Gravity.TOP, 0, 5 * (height / 8));
                toast.show();
                MediaPlayer mp = MediaPlayer.create(MakeFriend.this, R.raw.great);
                mp.start();
            }
        });
    ...
    private class MyTimerTask extends TimerTask {
        @Override
        public void run() {
            MakeFriend.this.runOnUiThread(new Runnable() {
                @Override
                public void run() {
